...
//                        }
            ...
            if ( "NOT".equals(array[0]) ) {
                if(ii.dictionary.containsKey(array[1])){
            ...
            for ( int i=start; i<array.length; i+=2 ) {
                if ( "AND".equals(array[i-1]) ) {
                    if ( !("NOT".equals(array[i])) ) {
                        if ( ii.dictionary.containsKey(array[i]) ) 
                        {
                            if(list != null)
                            list.retainAll(ii.dictionary.get(array[i]));
                        }
    ...
    public void printList ( ArrayList<Integer> list ) {
        ...
        public double processFreeTextQueries ( String[] array, Map<String, Double> IDFHashMap) {
        ArrayList<Double> values= new ArrayList<Double>();
        double EucDistance = 0;
            for (String word : array) {
                if(IDFHashMap.containsKey(word)){
                    values.add(Math.pow(IDFHashMap.get(word),2));
//                    values.add(IDFHashMap.get(word) * IDFHashMap.get(word));
                }
            }
            if(!values.isEmpty()){
